...
from utils import image_save, create_image_grid, natural_sort_key
...
def main():
    ...
    WDNet_dwv_path = os.path.join(args.input_path, 'WDNet', 'dwv', args.attack_type)
    SplitNet_dwv_path = os.path.join(args.input_path, 'SplitNet', 'dwv', args.attack_type)
    BVMR_dwv_path = os.path.join(args.input_path, 'BVMR', 'dwv', args.attack_type)
    WDNet_iwv_path = os.path.join(args.input_path, 'WDNet', 'iwv', args.attack_type)
    SplitNet_iwv_path = os.path.join(args.input_path, 'SplitNet', 'iwv', args.attack_type)
    BVMR_iwv_path = os.path.join(args.input_path, 'BVMR', 'iwv', args.attack_type)
    dwv_paths = [WDNet_dwv_path, SplitNet_dwv_path, BVMR_dwv_path]
    iwv_paths = [WDNet_iwv_path, SplitNet_iwv_path, BVMR_iwv_path]

    transform_norm = transforms.Compose([transforms.ToTensor()])
    tensor_to_pil = transforms.ToPILImage()

    grids = [[] for _ in range(args.num_of_images)]
    for dwv_path in dwv_paths:
    ...
    for i, pil_images in enumerate(grids):
        grid_image = create_image_grid(pil_images)
        grid_image.save(os.path.join(args.output_path, f'{i+1}-dwv-{args.attack_type}-grid.png'))
    

    grids = [[] for _ in range(args.num_of_images)]
    for iwv_path in iwv_paths:
    ...
    for i, pil_images in enumerate(grids):
        grid_image = create_image_grid(pil_images)
        grid_image.save(os.path.join(args.output_path, f'{i+1}-iwv-{args.attack_type}-grid.png'))


if __name__ == '__main__':
    argparser = argparse.ArgumentParser()
    argparser.add_argument('--input_path', type=str,default='./dataset/experiment/vaccine_wm')
    argparser.add_argument('--output_path', type=str,default='./output/experiment/grid')
    ...
    argparser.add_argument('--show', action='store_true', help='output grid or not')
